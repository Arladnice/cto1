...
export const decompress = (byteArray: Uint8Array, encoding: string = 'gzip'): Promise<string> => {
    ...
    return new Response(cs.readable)
        .arrayBuffer()
        .then((arrayBuffer: ArrayBuffer) => new TextDecoder().decode(arrayBuffer));
};
...
export function template(str, ...values) {
    ...
    if (safeValues.length === 1 && typeof safeValues[0] === 'object' && !Array.isArray(safeValues[0])) {
...
export function startWithCapitalizeLetter(string) {
    return string[0].toLocaleUpperCase() + string.slice(1);
}
export function startWithCapitalizeLowerLetter(string) {
    return string[0].toLocaleLowerCase() + string.slice(1);
}

/**
 * Compact number formatter (e.g. 1 300 → 1,3 тыс; 1 300 000 → 1,3 млн)
 * Falls back to ru-RU grouping if Intl compact is unavailable
 */
export function formatNumberCompact(value: number | null | undefined, maximumFractionDigits: number = 2): string {
    if (value == null || isNaN(Number(value))) return '0';

    const num = Number(value);
    try {
        // Most modern browsers support compact notation
        ...
        for (const u of units) {
            if (abs >= u.v) {
                const n =
                    Math.round((abs / u.v) * Math.pow(10, maximumFractionDigits)) / Math.pow(10, maximumFractionDigits);
                return sign + n.toString().replace('.', ',') + u.s;
            }
        }
