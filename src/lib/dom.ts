...
export const setStyle = (strStyle: string, cssObjectID = generateUID('st')) => {
...
const setProperty = (node: Element, arg: any): Element => {
    Object.entries(arg).forEach(([key, val]) => {
        if (key == 'to') {
            ...
            node.innerHTML = val as string;
        } else if (key == 'id') {
            node.id = val as string;
        } else if (key == 'class') {
            node.classList.value = (typeof val === 'string') ? val : (val as string[]).join(' ');
        } else {
            node.setAttribute(camelToKebab(key), val as string);
        }
    });
    return node;
}
